...
public class Ares extends Worker {
   ...
   */
  public void notifyWithRemovable(GameBoard gameBoard, Space unmovedWorkerPosition)
      throws IllegalArgumentException, IllegalStateException {
    if (gameBoard == null || !checkUnmovedWorkerPosition(unmovedWorkerPosition))
  ...
  private List<Space> getRemovableSpaces(GameBoard gameBoard, Space unmovedWorkerPosition) {
    ...
    for (int i = Constants.GRID_MIN_SIZE; i < Constants.GRID_MAX_SIZE; i++) {
      for (int j = Constants.GRID_MIN_SIZE; j < Constants.GRID_MAX_SIZE; j++) {
        ...
        if (canRemove(space, unmovedWorkerPosition)) {
          removable.add(space);
        }
      }
    }
  ...
  public boolean canRemove(Space space, Space unmovedWorkerPosition) {
    return neighbour(space, unmovedWorkerPosition)
        && space.isEmpty()
        && space.getTower().getHeight() > 0
  ...
  private boolean neighbour(Space space, Space unmovedWorkerPosition) {
    return (space.getRow() - unmovedWorkerPosition.getRow() < 2)
        && (unmovedWorkerPosition.getRow() - space.getRow() < 2)
        && (space.getColumn() - unmovedWorkerPosition.getColumn() < 2)
        && (unmovedWorkerPosition.getColumn() - space.getColumn() < 2)
        && (space.getRow() != unmovedWorkerPosition.getRow()
            || space.getColumn() != unmovedWorkerPosition.getColumn());
  }
  ...
  public boolean checkUnmovedWorkerPosition(Space unmovedWorkerPosition) {
    return ((unmovedWorkerPosition.getWorker().getWorkerColor().equals(this.workerColor))
        && ((unmovedWorkerPosition.getRow() != this.position.getRow())
            || (unmovedWorkerPosition.getColumn() != this.position.getColumn())));
  }
}
