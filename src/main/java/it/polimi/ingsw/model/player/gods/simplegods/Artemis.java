...
public class Artemis extends Worker {
   ...
   * @return boolean true if the worker moved correctly to space, false if the worker did not move
   *     to space.
  ...
  public boolean move(Space space) throws IllegalArgumentException {
    if (oldPosition == null) { // first move
      oldPosition =
          position; // save the actual position: Artemis can't move here again in this turn
  ...
  /**
   * Method isSelectable returns if the worker can move to the space received. This worker can move
   * twice but can not select oldPosition (the space where he was previously) as second move.
   *
  ...
  /**
   * Method notifyWithBuildable notifies the selectSpaceListener with all the spaces on which the
   * worker can build.
   *
