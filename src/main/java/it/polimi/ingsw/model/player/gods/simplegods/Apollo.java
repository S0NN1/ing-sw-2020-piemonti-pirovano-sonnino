...
public class Apollo extends Worker {
  ...
  public boolean isSelectable(Space space) throws IllegalArgumentException {
    ...
    boolean result =
        canMoveTo(space) && (space.getTower().getHeight() - position.getTower().getHeight() < 2);
    if (space.isEmpty()) return result;
    return !space.getWorker().getWorkerColor().equals(this.getWorkerColor()) && result;
  }

  /**
   * Method move changes the worker's position while check winning condition. This worker can also
   * change his position\ with a neighboring worker.
   *
   * @param space of type Space - the new position.
   * @throws IllegalArgumentException when space is null.
   * @return boolean value true if the worker moved correctly to the space received, false if he did
   *     not.
