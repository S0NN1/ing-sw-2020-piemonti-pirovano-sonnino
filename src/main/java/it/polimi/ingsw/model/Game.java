...
/**
 * Game class contains the main logic of "Santorini", which is divided in several macro-areas. The
 * first area is the Player/Worker section, which contains information about the single player and
 * his god power (overriding build and construct methods). The second area is the GameBoard section,
 * which contains a grid of 25 cells (Space class). That class contains information about the status
 * of the cell, and the presence of a tower in it. The last section is the "MVC Deck creation",
 * which consists in a Model-View-Controller pattern that lets challenger to choose the god powers
 * cards.
 *
...
public class Game {
  ...
  /**
   * Method createNewPlayer creates a new player in the match. The minimum length of activePlayers
   * array is 2 elements, and the maximum is 3.
   *
  ...
  /**
   * Method getPlayerByNickname searches the player identified by his nickname in the list of active
   * player.
   *
  ...
  public void nextPlayer() {
    currentPlayerN =
        (currentPlayerN == activePlayers.size() - 1 || currentPlayerN == activePlayers.size())
            ? 0
