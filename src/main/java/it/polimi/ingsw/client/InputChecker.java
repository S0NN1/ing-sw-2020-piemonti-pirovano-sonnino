...
/**
 * InputChecker class checks the correctness of the input received from the ActionParser, returning
 * either true or false.
 *
 * @author Luca Pirovano, Nicolò Sonnino
 */
public class InputChecker {
  public static final String ERR_NONEXISTENT_UNREACHABLE =
      "Non-existent or unreachable cell, operation not " + "permitted!";
  ...
  public static final String ERR_WORKER_NOT_SELECTED =
      "Worker not selected, operation not permitted!";
   ...
   *
   * @param connectionSocket of type ConnectionSocket - the socket used for the connection between
   *     client and server.
  ...
  /**
   * Method set validates a "SET &lt;x1&gt; &lt;y1&gt; &lt;x2&gt; &lt;y2&gt;" worker placement
   * message type.
   *
   * @param in of type String[] - the user input under array representation.
   * @return ChallengerPhaseAction - the correct ChallengerPhaseAction, null otherwise.
  ...
  public WorkerSetupAction set(String[] in) {
    ...
    try {
      ...
      if (row1 < Constants.GRID_MIN_SIZE
          || row1 >= Constants.GRID_MAX_SIZE
          || column1 < Constants.GRID_MIN_SIZE
          || column1 >= Constants.GRID_MAX_SIZE
          || row2 < Constants.GRID_MIN_SIZE
          || row2 >= Constants.GRID_MAX_SIZE
          || column2 < Constants.GRID_MIN_SIZE
   ...
   * @param activeWorker of type int - the number indicating which worker was selected by player at
  ...
  public BuildAction build(int turnPhase, int row, int column, int activeWorker) {
    ...
    if (turnPhase == 2
        || Constants.getBuildPhaseGods().contains(modelView.getGod().toUpperCase())) {
  ...
  /**
   * Method canReachCell checks if cell is reachable (no dome built or higher than current cell of
   * one level).
   *
   * @param row of type int - the row of the selected cell.
   * @param column of type int - the column of the selected cell.
  ...
  private MoveAction canReachCell(int row, int column, Couple worker, MoveAction move) {
    if (modelView.getBoard().getGrid()[row][column].isDome()) {
      ...
      if (modelView.getBoard().getGrid()[row][column].getLevel()
              - modelView.getBoard().getGrid()[worker.getRow()][worker.getColumn()].getLevel()
          >= 2) {
        System.out.println(
            RED + "Trying to move up to unreachable level, operation not permitted!" + RST);
        return null;
      } else return move;
    }
  }
  ...
  /**
   * Method canMoveToOccupiedCell checks if moving to occupied cell is permitted by checking if the
   * player's god is one with this particular effect.
   *
   * @param move of type MoveAction - the MoveAction parsed.
   * @return MoveAction - the correct MoveAction, null otherwise.
   */
  ...
  /**
   * Method getBuildAction checks if BuildAction isn't permitted by firing various errors based on
   * their types.
   *
   * @param row of type int - the row of the selected cell.
   * @param column of type int - the column of the selected cell.
  ...
  private BuildAction getBuildAction(int row, int column, Couple worker, BuildAction build) {
    if (isUnreachable(row, column, worker)) {
      ...
      if (modelView.getBoard().getGrid()[row][column].getColor() != null
          && !Constants.getBuildSameBlockGods().contains(modelView.getGod().toUpperCase())) {
        System.out.println(RED + ERR_CELL_OCCUPIED + RST);
        return null;
      } else {
  ...
  /**
   * Method isUnreachable checks if selected cell is within a distance of 1 cell from the worker or
   * if the player is trying to access outside the grid.
   *
   * @param row of type int - the row of the selected cell.
   * @param column of type int - the column of the selected cell.
   * @param worker of type Couple - worker's position.
  ...
  private boolean isUnreachable(int row, int column, Couple worker) {
    return row < Constants.GRID_MIN_SIZE
        || row >= Constants.GRID_MAX_SIZE
        || column < Constants.GRID_MIN_SIZE
        || column >= Constants.GRID_MAX_SIZE
        || row >= Objects.requireNonNull(worker).getRow() + 2
        || row <= worker.getRow() - 2
        || column >= worker.getColumn() + 2
        || column <= worker.getColumn() - 2;
  }
  ...
  public BuildAction removeLevel(int turnPhase, int row, int column, int activeWorker) {
    ...
    if (turnPhase == 3 && Constants.getEndActionGods().contains(modelView.getGod().toUpperCase())) {
      return build;
    } else {
      System.err.println(ERR_INCORRECT_ACTION);
      return null;
  ...
  public SelectBuildAction selectRemoveLevel(int turnPhase, int activeWorker) {
    if (activeWorker == 0) {
      ...
      return new SelectBuildAction(Action.SELECT_REMOVE);
    } else {
      System.err.println(ERR_INCORRECT_ACTION);
      return null;
    }
