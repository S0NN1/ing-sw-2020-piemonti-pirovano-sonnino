...
class ActionControllerTest {
  ...
  /**
   * Method selectMoveActionTest tests the method readMessage with a SelectMoveAction and different
   * phase's values.
   */
  @Test
  ...
  /**
   * Method selectBuildActionTest tests the method readMessage with a SelectBuildAction and
   * different phase's values.
   */
  @Test
  ...
  /**
   * Method moveActionTest tests the method readMessage with a MoveAction and different phase's
   * value.
   */
  @Test
  ...
  void moveActionTest() {
    ...
    // test right behavior: right message on right phase (space where to move is empty -> normal
    // move)
  ...
  /**
   * Method forceWorkerTest tests the methods readMessage when different messages arrives. In
   * particularly, it tests readMessage during SELECT_FORCE_WORKER and FORCE_WORKER Charon's phases.
   */
  @Test
  ...
  void removeActionTest() throws OutOfBoundException {
    ...
    Space unmovedSpace =
        gameBoard.getSpace(4, 2); // creates an instance of Ares and Ares unmoved worker
    ...
    SelectBuildAction selectBuildNormal = new SelectBuildAction();
    assertFalse(
        actionController.readMessage(selectBuildNormal, unmovedSpace), "3a"); // try to invoke
  ...
  /**
   * Method sequenceActionsTest tests a sequence of actions: SELECT_MOVES, MOVE, SELECT_BUILD,
   * BUILD.
   */
  @Test
  ...
  void sequenceActionsTest() {
    ...
    assertFalse(
        actionController.readMessage(message1), "2b"); // try to make a forbidden action for phase's
    // value
  ...
  void endActionTest() {
    ...
    // test wrong behaviour: after read a SelectBuildAction, BuildAction is a must -> turn can't end
    // now
    actionController.setPhase(4);
