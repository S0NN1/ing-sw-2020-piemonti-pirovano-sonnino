...
class GodSelectionControllerTest {
    ...
    private static class VirtualClientStub extends VirtualClient {
        ...
        public void propertyChange(PropertyChangeEvent evt) {
            ...
            if (evt.getNewValue() instanceof ChallengerMessages) {
                if (((ChallengerMessages)evt.getNewValue()).getMessage()!=null) message = ((ChallengerMessages)
                        evt.getNewValue()).getMessage();
  ...
  private static class GameHandlerStub extends GameHandler {
        ...
        public void sendAllExcept(Answer message, int excludedID) {
            System.out.println(Constants.ANSI_RED + "\nSemi-broadcast message: " + Constants.ANSI_RESET +
                    message.getMessage());
        }
        ...
        public void sendAll(Answer message) {
            System.out.println(Constants.ANSI_RED + "\nFull-broadcast message: " + Constants.ANSI_RESET +
                    message.getMessage());
        }
    ...
    final GodSelectionController selectionController = new GodSelectionController(new CardSelectionModel(
            game.getDeck()), controller, virtualClient);
    ...
    void selectionFlowTest() {
        ...
        selectionController.propertyChange(new PropertyChangeEvent(this, null, null,
                new ChallengerPhaseAction("LIST", null)));
        assertTrue(virtualClient.notified);
        assertEquals(Card.godsName(), virtualClient.gods);
        virtualClient.notified = false;
        selectionController.propertyChange(new PropertyChangeEvent(this, null, null,
                new ChallengerPhaseAction("DESC", Card.APOLLO)));
        assertTrue(virtualClient.notified);
