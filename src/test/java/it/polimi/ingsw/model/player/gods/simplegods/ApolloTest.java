...
class ApolloTest {
  ...
  void getMovesTest() {
    ...
    worker3.setPosition(
        gameBoard.getSpace(2, 1)); // selectMoves shouldn't return this space because
  ...
  /**
   * Method listenerTest tests double move listener which is fired when Apollo makes a move into a
   * not empty space.
   */
  @Test
  ...
  void listenerTest() {
    ...
    assertEquals(
        apollo.getPosition().getColumn(), client.otherMove.getOldPosition().getColumn(), "6");
    assertEquals(worker.getPosition().getRow(), client.otherMove.getNewPosition().getRow(), "7");
    assertEquals(
        worker.getPosition().getColumn(), client.otherMove.getNewPosition().getColumn(), "8");
  }
